...
public class PasswordUpdateSave extends ActionSupport {
	...
	public String execute() throws Exception {
		...
		/*
		 * else if(type.equals("3"))//学生身份 { //查询原密码是否正确 if (new
		 * StudentDao().CheckPassword(session.getAttribute("id").toString(),
		 * Password)) { //修改密码 StudentBean cnbean=new StudentBean(); cnbean=new
		 ...
		 * print("<script language='javascript'>alert('修改成功！');window.location='PasswordUpdate.jsp';</script>"
		 * ); out.flush();out.close();return null; } else { Msg = "用户名或者密码错误";
		 * return INPUT; } }
		 */
		else {
			out.print("<script language='javascript'>alert('请重新登录！');window.location='Login.jsp';</script>");
